...
class VCMITCPServer:
    ...
    def _handle_client(self, client_socket: socket.socket, client_address: tuple, client_id: str):
        ...
        # Per-connection string interning state - the C++ serializer maintains
        # string interning across multiple sendPack calls on the same connection
        loaded_strings = []
        ...
        try:
            while self.running:
                try:
                    ...
                    pack = deserialize_pack_with_strings(data, loaded_strings)
                    if pack is None:
